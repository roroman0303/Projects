...
server_files = ['Importantitem.txt','ItemMoveInfoV4.txt',
                ...
                'Collection.lua','NotDisappearAfterUsingItemList.lua', 'LapineDdukDDakBox.lua', 'CashItemList.txt']
...
korean_luckybox_files = ['special.sc', 'combiItem.sc', 'idnum2itemresnametable.txt',
                         'num2itemresnametable.txt', 'itemslotcounttable.txt', 'script.sql']

# === создание sql-запросов из корейского Excel файла ===
def create_sql(region, korean_doc='korean_doc.xlsx'):
    table = files.open_table(filename=korean_doc, sheetname='database(en)')
    # for row in range(len(table)): print(table[row]) # для проверки
    sql_rows = list()
    ...
    for row in table:
        ...
        else: sql_rows.append(db.create_row(region, current_table, row))
    sql_text = '\n'.join(sql_rows)
    return sql_text

# === создание словаря с текстом файлов из корейского Excel файла ===
def create_files(region, korean_doc='korean_doc.xlsx', for_luckybox=False):
    korean_dict = dict()
    korean_dict.update(create_server(korean_doc))
    korean_dict.update(create_client(korean_doc))
    korean_dict['script.sql'] = create_sql(region, korean_doc)
    if for_luckybox == True:
        luckybox_dict = dict()
        for file in korean_luckybox_files:
            if korean_dict.get(file) is not None: luckybox_dict[file] = korean_dict[file]
        return luckybox_dict
    else: return korean_dict
...
def create_server(korean_doc='korean_doc.xlsx'):
    ...
    for row in range(len(table)):
        if table[row][0] is not None and table[row][1] is None:
            ...
            elif 'cashitemlist.txt' in table[row][0].lower(): table[row][0] = 'CashItemList.txt'
