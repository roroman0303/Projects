...
if (thx_3364 == 0)
	...
	case 2
		...
		wait
		dialog "[Лана]"
		dialog "Если вы сделаете то, о чём я рассказал, вы станете частью фестиваля!"
		wait
		dialog "[Лана]"
		dialog "^0000cdСмоляные ветки^000000 можно получить с Оживших веток."
		setquest 3364
		close
		return
	break
...
elseif (thx_3364 == 1)
	...
	if ((v[Fire_Arrow] > 0) & (v[Tree_Root] > 9) & (v[Goodly_Bough] > 4))
		...
		wait
		dialog "[Лана]"
		dialog "Первый - билет на корабль, <NAVI>[Капитан корабля]<INFO>alberta.gat,140,170,0,101,0</INFO></NAVI> может отправиться с вами туда, куда вы попросите."
		wait
		dialog "[Лана]"
...
//==============================================================================	
	...
	endchoose
	dialog "[Помощник ведущего]"
	dialog "Вообще-то, я бы не отказался от ^0000cd10 Фестивального печенья^000000. Я мог бы предложить поучаствовать в лотерее за них."
	wait
	...
	case 1
		if (v[Harvest_Cookie] > 9)
			...
			dialog "[Помощник ведущего]"
			dialog "У вас нет ^0000cd10 Фестивального печенья^000000."
			close
			return
		endif
	break
