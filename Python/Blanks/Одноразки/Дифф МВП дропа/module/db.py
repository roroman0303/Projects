...
# запрос в таблицу, поиск по первому столбцу
def select_one(region, table, cell):
    ...
    if first_column is not None:
        if str(cell).isdigit() == True:
            query = 'select * from [script].[dbo].[{}] where [{}] = {}'.format(table, first_column, cell)
            try:
...
def insert(table, row):
    ...
    query = 'INSERT INTO [script].[dbo].[{}] VALUES {}'.format(table, parameters)
    if "\"" in query: query = query.replace("\'", '\'\'').replace("\"", '\'')  # исправление кавычек под sql
    return query

# найти предмет в БД по id
def find_table(region, id):
    for table in dbitems.all_tables:
        db_row = select_one(region, table, id)  # получаем существующую строку в БД
